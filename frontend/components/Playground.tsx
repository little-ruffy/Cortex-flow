...
import { Send, Activity, CheckCircle, Clock } from "lucide-react";
...
export default function Playground() {
    ...
    const fetchStats = async () => {
        try {
            const res = await axios.get("http://localhost:8000/api/v1/admin/analytics?source=playground");
            setStats(res.data);
        } catch (error) {
            console.error(error);
        }
